...
class VanillaRNN(object):
	...
	def step_backward(self, dnext_h, meta):
		...
		dWx: gradients w.r.t. feature-to-hidden weights (D, H)
		dWh: gradients w.r.t. hidden-to-hidden weights (H, H)
...
class LSTM(object):
	...
	def step_forward(self, x, prev_h, prev_c):
		...
		meta: variables needed for the backward pass
		"""
		output = x.dot(self.params[self.wx_name]) + prev_h.dot(self.params[self.wh_name])  + self.params[self.b_name]
		H = self.h_dim      
		sigmoid = lambda z:  np.exp(z)/(1.0 + np.exp(z)) 
		i = sigmoid(output[:,0:H])
		f = sigmoid(output[:,H:2*H])
		o = sigmoid(output[:,2*H: 3*H])
		g = np.tanh(output[:,3*H: 4*H])      
		next_c = f * prev_c + i * g
		intermediate_step = np.tanh(next_c)
		next_h = o * intermediate_step
		meta = [next_c, prev_c, output, x, prev_h, next_h, intermediate_step]
	...
	def step_backward(self, dnext_h, dnext_c, meta):
		...
		"""
		next_c, prev_c, output, x, prev_h, next_h, intermediate_step = meta       
		H = self.h_dim
		i = sigmoid(output[:, 0:H])
		f = sigmoid(output[:, H:2*H])
		o = sigmoid(output[:, 2*H: 3*H])
		g = np.tanh(output[:, 3*H: 4*H])        
		dtanh = o * dnext_h
		do = intermediate_step * dnext_h
		doz = do * (o * (1.0 - o))
		dc =  dtanh*(1.0-intermediate_step**2)
		dc += dnext_c        
		df =  dc * prev_c
		dfz = df * (f*(1.0-f))        
		di = dc * g
		...
		dgz = dg*(1.0 - g**2)
		dprev_c = f *  (dc)
		dA = np.hstack((diz, dfz, doz, dgz))
		dWx = x.T.dot(dA)
		dWh = prev_h.T.dot(dA)
		dx = (self.params[self.wx_name].dot(dA.T)).T
		db = np.sum(dA, axis=0)
		dh = self.params[self.wh_name].dot(dA.T).T        
		#############################################################################
		# TODO: check out the one step backward pass implementation				    #
		#############################################################################
		#x, prev_h, next_h = meta
		...
		return dx, dh, dprev_c, dWx, dWh, db
